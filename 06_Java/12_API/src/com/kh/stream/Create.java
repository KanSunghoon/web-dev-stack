 ...
 * 
 * * 스트림의 종류
 * - java.util.stream 패키지에 존재하고 BaseStream 인터페이스를 부모로 갖고 자식 인터페이스들이 상속 관게를 이루고 있다.
 * 
 * * Stream API
...
public class Create {
	...
	// 숫자 범위로 스트림 생성 - IntStream, LongStream, DoubleStream
	public void method1() {
		
		// range : 첫 번째 파라미터값 ~ 두 번째 파라미터값 이전까지의 값을 요소로 가지는 스트림 객체 생성
		IntStream stream = IntStream.range(1, 10); // 1 ~ 9
		int sum = stream.sum();
		System.out.println("range의 sum : " + sum);
		
		// rangeClosed : 첫 번째 파라미터값 ~ 두 번째 파라미터값까지의 값을 요소로 가지는 스트림 객체 생성
