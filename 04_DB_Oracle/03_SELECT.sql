...
/*
    ...
    >, <, >=, <= : 대소 비교
*/
-- USER_INFO에서 나이가 30이상인 
-- 이름(NAME), 나이(AGE), 생년월일(BIRTHDATE) 조회
SELECT NAME, AGE, BIRTHDATE
FROM USER_INFO
WHERE AGE >= 30;

-- EMPLOYEE에서 부서코드(DEPT_CODE)가 
--                  'D9'인 사원들 (모든 컬럼) 조회
SELECT *
FROM EMPLOYEE
WHERE DEPT_CODE = 'D9';
...
-- EMPLOYEE에서 보너스(BONU)를 받지 않는 사원들 조회
SELECT *
FROM EMPLOYEE
WHERE BONUS IS NULL;
...
-- 직급코드(JOB_CODE)가 J7이거나 J2인 사원들 중 급여가 200만원인 사원들 조회 
-- IN 연산자 사용 / OR 연산자 
SELECT * 
FROM EMPLOYEE
...
SELECT DEPT_CODE, TO_CHAR(FLOOR(AVG(NVL(SALARY, 0))), '999,999') "300 이상 평균급여"
FROM EMPLOYEE
