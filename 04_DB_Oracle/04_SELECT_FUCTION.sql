...
SELECT
    LTRIM('     K H     '),  -- 왼쪽 공백 제거
    LTRIM('ACBAABBBKH', 'ABC'), -- 왼쪽에서 ABC에 해당하는 문자들 제거
...
ALTER SESSION SET NLS_DATE_FORMAT = 'YYYYY-MM-DD';
...
/*
    ADD_MONTHS(날짜, 숫자)
    - 특정 날짜에 입력받는 숫자만큼 개월 수를 더한 날짜 변환
*/
...
SELECT TO_DATE(BIRTHDATE, 'YYYY')
    || '년'
    || TO_DATE(BIRTHDATE, 'MM')
    || '월'
    || TO_DATE(BIRTHDATE, 'DD')
    || '일'
...
*/
SELECT NULLIF('123', '123'), NULLIF('1,2,3', '456')
FROM DUAL;
...
-- 그 외 직급 사원은 급여를 5% 인상
-- 정렬 : 직급코드(JOB_CODE) J1 부터, 인상된 급여 높은 순서대로
SELECT 
    JOB_CODE, SALARY,
    DECODE(JOB_CODE, 'J7', SALARY * 1.1, 
                     'J6', SALARY * 1.15, 
                     'J5', SALARY * 1.2, SALARY * 1.05) "인상된 급여"
FROM EMPLOYEE
...
SELECT JOB_CODE, SUM(SALARY)
FROM EMPLOYEE
GROUP BY CUBE(JOB_CODE); -- 컬럼 하나일 때는 ROLLUP과 차이 X
